#include <iostream>
#include <thread>
#include "CPU_Raytracer/thread_pool.h"

ThreadPool::ThreadPool(size_t numThreads) : stop(false), unfinishedTasks(0) {
    for (size_t i = 0; i < numThreads; ++i) {
        workers.emplace_back(&ThreadPool::workerThread, this);
    }
}

ThreadPool::~ThreadPool() {
    {
        std::unique_lock<std::mutex> lock(queueMutex);
        stop = true;
    }
    condition.notify_all();
    for (std::thread& worker : workers) {
        worker.join();
    }
}

void ThreadPool::enqueue(std::function<void()> func) {
    {
        std::unique_lock<std::mutex> lock(queueMutex);
        tasks.push(func);
        ++unfinishedTasks;
    }
    condition.notify_one();
}

void ThreadPool::workerThread() {
    while (true) {
        std::function<void()> task;
        {
            std::unique_lock<std::mutex> lock(queueMutex);
            condition.wait(lock, [this] { return stop || !tasks.empty(); });
            if (stop && tasks.empty()) {
                return;
            }
            task = std::move(tasks.front());
            tasks.pop();
        }
        {
            std::unique_lock<std::mutex> lock(outputMutex); // Lock for output
           // std::cout << "Thread " << std::this_thread::get_id() << " is executing task." << std::endl;
            task();
           // std::cout << "Thread " << std::this_thread::get_id() << " has completed task." << std::endl;
        }
        {
            std::unique_lock<std::mutex> lock(queueMutex);
            --unfinishedTasks;
            if (unfinishedTasks == 0) {
                doneCondition.notify_all();
            }
        }
    }
}

void ThreadPool::waitUntilDone() {
    std::unique_lock<std::mutex> lock(queueMutex);
    doneCondition.wait(lock, [this] { return unfinishedTasks == 0; });
}

